...
int test,num,a,b,arr[2000],r1,r2;
vector <long long int>mx,mn;
cin>>test;
while(test--){
        r1=100000000;
  r2=100000000;
    ...
    FOR(i,0,b){
    cin>>arr[i];
    if(arr[i]>a)mn.PB(arr[i]);
    else mx.PB(arr[i]);
    }
    vector<long long int> :: iterator it;
    RFOR(i,1,a){
      it = find(mn.begin(), mn.end(), i);
       if(it==mn.end()){
        r1=abs(i-a);
        break;
    }
    ...
    cout<<r1<<endl;
    int x=min(a+1001, num+1);
    FOR(i,a,x){
    it=find(mx.begin(),mx.end(),i);
    if(it==mx.end()){
        r2=abs(i-a);
            cout<<r2<<endl;
    break;
